...
export async function fuzzerMain(
  ...
  const verify = args['v'];
  const showOnlyFailingCases = args['f'] === true || args['failed'] === true;
  ...
  const help =
    args['h'] === true || args['help'] === true || args['?'] === true;
...
function showUsage(
  ...
  console.log('');
  console.log(
    `Usage: node ${program} [-d datapath] [-n count] [-o outfile] [-v] [-h|help|?]`
  );
  console.log('');
...
export async function runFuzzer(
  ...
  const name =
    generatorName === undefined
      ? testCaseGeneratorFactory.getDefaultName()
      : generatorName;
...
export async function runTests(
  ...
  for (const test of tests) {
    ...
    count++;
    const result = await test.run(processor, catalog, CorrectionLevel.STT);
...
export class TestCaseGeneratorFactory {
  ...
  constructor(generators: TestCaseGeneratorDescription[]) {
    if (generators.length < 1) {
      const message =
        'TestCaseGeneratorFactory: expect at least one generator.';
