...
import { loadShortOrderWorld, ShortOrderWorld } from '../integration';
    ...
    repl.getReplServer().defineCommand('query', {
      ...
      action: (query: string) => {
        if (this.prefix.length < 1) {
          console.log(
            'No prefix set. First set a prefix string with the .prefix command.'
          );
        } else {
          console.log(`token-flow query = ${query}`);

          const termsPrefix = this.lexer.lexicon.termModel.breakWords(query);
          const stemmedPrefix = termsPrefix.map(
            this.world2.lexer.lexicon.termModel.stem
          );
          const hashedPrefix = stemmedPrefix.map(
            this.world2.lexer.lexicon.termModel.hashTerm
          );

          const termsQuery = this.lexer.lexicon.termModel.breakWords(query);
          const stemmedQuery = termsQuery.map(
            this.world2.lexer.lexicon.termModel.stem
          );
          const hashedQuery = stemmedQuery.map(
            this.world2.lexer.lexicon.termModel.hashTerm
          );

          const isDownstreamTerm: DownstreamTermPredicate<number> = (
      ...
      action: (line: string) => {
        ...
        for (const [i, edges] of filteredGraph.edgeLists.entries()) {
          ...
          for (const edge of edges) {
            ...
            console.log(
              `    length:${edge.length}, score:${edge.score.toFixed(
                2
              )}, token:${token}`
            );
          }
